...
* **Dual-Tier Archiving (Cold Storage)**: Dynamically ferry expired local backups (Hot Storage) to cheap, high-capacity NFS or cloud mount points (Cold Storage). Native support for global fallbacks, job-level path routing, and strict chain-retention pruning.
...
* **Unified Multi-Channel Notifications**: Built-in async notification dispatcher supporting Microsoft 365 Graph API (Native OAuth2 HTML Email SaaS cards), SMTP, Discord Webhooks, and Generic Webhooks.
...
* **Live TUI Log Streaming**: Trigger backup jobs manually from the CLI and instantly attach to the daemon's live log stream via an interactive Bubble Tea terminal UI to watch the execution in real time.
...
1. **`gobackupd` (The Daemon)**: A background service that holds an embedded database for authentication, natively runs the Cron engine, manages snapshot states, orchestrates multi-tier archiving, and executes the heavy-lifting SSH tar backup processes.
...
# Configure a job with dual-tier archiving
